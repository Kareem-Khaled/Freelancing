...
def run(mail, psw, sheet_name, delay):
    try:
        ...
        if len(code):
            try: 
                ...
                aprv = driver.find_element(By.ID, 'approvals_code')
                aprv.send_keys(code)
    ...
    while i < len(groups):
        try:
            ...
            addPost = WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.XPATH, "/html/body/div[1]/div/div[1]/div/div[3]/div/div/div/div[1]/div[1]/div[4]/div/div/div/div/div/div[1]/div[1]/div/div/div/div[1]/div")))   
