...
try:
    ...
    driver.find_element(By.CLASS_NAME, 'nc_username').send_keys(username)
    driver.find_element(By.CLASS_NAME, 'nc_password').send_keys(password)
    _ = WebDriverWait(driver, 50).until(
        EC.presence_of_element_located((By.CLASS_NAME, "nc_login_submit")))
    actions = ActionChains(driver)
    actions.move_to_element(driver.find_element(By.CLASS_NAME, 'nc_login_submit')).click().perform()
...
try:
    for line in open("DNS Sequence.csv"):
        while True:
            try:
                actions = ActionChains(driver)
                actions.move_to_element(driver.find_element(By.XPATH, '/html/body/div[1]/div[3]/div/div/div[2]/div[5]/div/div[3]/div[1]/div[2]/div[2]/div[2]/div/table/tbody[3]/tr/td/div[1]/a[1]')).click().perform()
                break
